...
def runscript(timeout, thread_limit, number_processes):
	...
	return f"mpiexec --timeout {int(timeout)} -x NNODES=40 -x SEED={SEED} -x OMP_THREAD_LIMIT={thread_limit}  -x  OMP_NESTED=true -n {number_processes}  cmake-build-debug/cppprojct > tmp/trash.txt"
...
if __name__=='__main__':
	...
	CHARACTERISTIC_TIME = 10 #30 SECONDS initial guess
	#start = time.time()
	...
	ax[0].scatter(*zip(*sorted([(NTHREADSMAX_v[i], times[i]) for i in range(len(times))], key=lambda tup: tup[0])), c='r')
	ax[1].scatter(*zip(*sorted([(NPROCS_v[i], times[i]) for i in range(len(times))], key=lambda tup: tup[0])), c='b')
	ax[0].plot(*zip(*sorted([(NTHREADSMAX_v[i], times[i]) for i in range(len(times))], key=lambda tup: tup[0])), c='r')
	ax[1].plot(*zip(*sorted([(NPROCS_v[i], times[i]) for i in range(len(times))], key=lambda tup: tup[0])), c='b')
	...
	ax[2].scatter(*zip(*sorted([(NPROCS_v[i]*NTHREADSMAX_v[i], times[i]) for i in range(len(times))], key=lambda tup: tup[0])), c='k')
	ax[2].plot(*zip(*sorted([(NPROCS_v[i]*NTHREADSMAX_v[i], times[i]) for i in range(len(times))], key=lambda tup: tup[0])), c='k')
